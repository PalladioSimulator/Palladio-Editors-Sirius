...
/**
 * Provides an anchor whith 1/3 offset of the oriented sides center
 * 
 * @author Jonas Lehmann
 */
public class InfrastructureRequiredRoleAnchorProvider implements AnchorProvider {
	...
	private static class OperationRequiredRoleAnchor extends AbstractOrientedAnchor {
		...
		protected Map<Orientation, Translation> createTranslationCalculators() {
			...
			calculators.put(Orientation.LEFT, (p, fb) -> {
				return fb.getTopLeft().translate(fb.width() / 3, fb.height() / 2);
			});
			calculators.put(Orientation.RIGHT, (p, fb) -> {
				return fb.getTopRight().translate(-fb.width() / 3, fb.height() / 2);
			});
			calculators.put(Orientation.BOTTOM, (p, fb) -> {
				return fb.getBottomLeft().translate(fb.width() / 2, -fb.height() / 3);
			});
			calculators.put(Orientation.TOP, (p, fb) -> {
				return fb.getTopLeft().translate(fb.width() / 2, fb.height() / 3);
			});
			return calculators;
		}
	}
