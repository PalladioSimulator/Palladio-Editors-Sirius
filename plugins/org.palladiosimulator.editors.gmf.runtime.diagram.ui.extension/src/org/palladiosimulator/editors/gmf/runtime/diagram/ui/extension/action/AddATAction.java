...
public class AddATAction implements IExternalJavaAction {
    ...
    /**
     * The name of the parameter to be given. This is an arbitrary object from the editor to get the
     * corresponding {@link Session}.
     */
    ...
    public void execute(Collection<? extends EObject> selections, Map<String, Object> parameters) {
        ...
        final ResourceSet resourceSet = SessionManager.INSTANCE.getSession(system).getTransactionalEditingDomain()
                .getResourceSet();

        final ArchitecturalTemplateSelectEObjectDialog atSelectionDialog = new ArchitecturalTemplateSelectEObjectDialog(
                PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), resourceSet,
                ArchitecturalTemplateSelectEObjectDialog.Type.AT);
        if (atSelectionDialog.open() == Dialog.OK && atSelectionDialog.getResult() != null) {
            ...
            final Optional<Stereotype> matchingStereotype = system
                    ...
                    .stream()
                    .filter(applicableStereotype -> at.getRoles().stream()
                            .anyMatch(role -> role.getStereotype().getName().equals( // FIXME:
                                                                                     // should be
                                                                                     // comparing
                                                                                     // the objects,
                                                                                     // equals is
                                                                                     // not
                                                                                     // implemented
                                    applicableStereotype.getName()))).findFirst();
            ...
            if (matchingStereotype.isPresent()) {
                ...
                // TODO check whether more explicit saving is needed (I guess, we just have to save
                // via the normal editor save operation) [Lehrig]
